...
func TestCaptureAllInboundTrafficWithExcludePorts(t *testing.T) {
	f, err := ioutil.TempFile("./", "iptables.*")
	assert.Nil(t, err)
	defer func() {
		assert.Nil(t, f.Close())
	...
	expect := []string{
		...
		"iptables -t nat -A APISIX_INBOUND -p tcp --dport 15010 -j RETURN",
		"iptables -t nat -A APISIX_INBOUND -p tcp --dport 15011 -j RETURN",
...
func TestCaptureOutboundTrafficWithExcludedPorts(t *testing.T) {
	f, err := ioutil.TempFile("./", "iptables.*")
	assert.Nil(t, err)
	defer func() {
		assert.Nil(t, f.Close())
